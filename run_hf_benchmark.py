...
Run with: uv run --python 3.11 run_hf_benchmark.py -m <model> -b <benchmark>
"""
...
def _load_scieval(save_dir: str, topics: list[str], abilities: list[str]) -> pl.DataFrame:
    ...
    df = df.group_by("question", maintain_order=True).agg([pl.col(c).drop_nulls().first().alias(c) for c in agg_cols])
    df = df.with_columns(pl.col("question").str.strip_chars().str.strip_suffix("Answer:").str.strip_chars())
    if topics:
...
def _patch_chatglm_source() -> None:
    """Patch the HF-cached chatglm tokenizer source (idempotent)."""
    ...
    pattern = os.path.expanduser("~/.cache/huggingface/modules/transformers_modules/**/tokenization_chatglm.py")
...
def _generate_on_gpu(
    ...
    tokenizer, model = _load_chatglm(model_id, gpu) if chatglm else _load_standard(model_id, gpu)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--subset", default="college_physics", choices=MMLU_SUBSETS, help="MMLU subset (ignored for scieval)"
    )
    ...
    output_path = os.path.join(
        output_dir, f"{args.model.replace('/', '-')}_{prompt_fn.__name__[1:]}_{timestamp}.parquet"
    )
