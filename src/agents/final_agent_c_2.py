...
agent_cfg = load_yaml("config/final_agent_c_2.yaml")
...
class PhysicsPlanningRetrievalComputeAgent:
    ...
    def _execute(self, state: State) -> State:
        ...
        ctx = self._work_ctx(state)
        if state.get("allow_compute", True) and agent_cfg.get("calculator_reference"):
            ctx = ctx + [HumanMessage(content=agent_cfg["calculator_reference"])]
        ctx = ctx + [HumanMessage(content=prompt_content)]
        ai = self.llm.invoke(ctx)
    ...
    def _resolve_answer(self, content: str, state: State) -> str:
        chosen = self._extract_letter(content)
        if chosen is None:
            options = self._parse_options(state.get("options", ""))
            chosen = sorted(options)[0] if options else "A"
            log.info(f"[FINALIZE] No letter from model; defaulting to {chosen}.")
